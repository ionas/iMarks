...
== Manage YOUR bookmarks in YOUR cloud.
* Setup the rails server anywhere you want, even at localhost or via DynDNS.
* Tested on Ruby 1.8.7. I recommend Ruby Version Manager: http://rvm.beginrescueend.com/ 
* After download be sure to run "bundle" and then "rails s"
...
* Bookmark HABTM Bookmark (for creating bookmark relations / kind of collections)
* Bookmark collections (like tags)
...
=== Chrome Client
* Add Bookmark Icon which reads <title> and applys this (optional) title to the bookmark. Copy chromes bookmark icon funcionality, add tags field.
* Autocompletion for tags
* Change icon depending on if the server is reachable
* Option: Disable Chromes internal bookmark star

=== Server
* Some stylesheet routes for non-extension access (via web browser)
